...
async function onPhaseChange(phase) {
  if (phase === "betting") {
    ...
    const input = await rl.question(
      ">> Betting is open! Type 'bet <amount>': ",
    );
    activeGame.handleInputEvent({
      type: "place_bet",
      amount: parseInt(input.split(" ")[1]),
    });
...
function startRocket(blastOffTime) {
  ...
  const interval = setInterval(() => {
    ...
    process.stdout.write(
      `\r\x1b[K>> Multiplier: ${multiplier.toFixed(2)}x | Press 'c' to cash out!`,
    );
...
async function startProgram() {
  ...
  while (true) {
    ...
    try {
      if (roomCode === "") {
        console.log("\n[1] Join a room\n[2] Create a room\n[3] Exit");
        userInput = await rl.question("Enter your command Number: ", {
          signal: menuAbortController.signal,
        });
        ...
        switch (userInput) {
          case "1":
            await checkUsername();
            const temp_roomCode = await rl.question(
              "Enter the room code you want to join: ",
            );
            console.log("Joining room...");
            const joinStatus = await postData("/join_room", {
              code: temp_roomCode,
              username: userName,
              sid: socket.id,
            });
            ...
            if (joinStatus["status"] === "success") {
              ...
              serverTimeOffset = joinStatus["Server Time"]
                ? joinStatus["Server Time"] - Date.now() / 1000
                : 0;
            } else if (joinStatus["status"] === "nameConflict") {
              console.log(
                "Name is taken in this current room, please try a different username.",
              );
              userName = "";
            }
            break;
          ...
          case "2":
            ...
            const createResponse = await postData("/create_room", {
              username: userName,
              sid: socket.id,
            });
            ...
            serverTimeOffset =
              createResponse["Server Time"] - Date.now() / 1000;
            break;
        ...
        switch (userInput) {
          case "1":
            const gameResponse = await postData("/select_game", {
              code: roomCode,
              game_id: "Crash Out",
            });
          ...
          case "3":
            const statusResponse = await postData("/room_status", {
              code: roomCode,
              username: userName,
            });
